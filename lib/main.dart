...
class _StudentsState extends State<StudentsPage> {
  ...
  Widget _studentTile(BuildContext c, Student s) => Container(
  ...
  Widget _sectionHeader(String label, int count) => Padding(
        ...
        child: Row(
          children: [
            ...
            Text('($count)', style: const TextStyle(color: Colors.black45, fontSize: 12)),
          ],
        ),
      );
  ...
  Widget build(BuildContext c) {
    ...
    final chips = ['الكل', ...widget.store.sections];

    Widget listArea;
    if (filterSection != 'الكل') {
      final list = searched.where((s) => s.section == filterSection).toList();
      listArea = list.isEmpty
          ? const Empty(icon: Icons.person_search, text: 'لا توجد نتائج.')
          : ListView(
              padding: const EdgeInsets.symmetric(horizontal: 12),
              children: [for (final s in list) _studentTile(c, s)],
            );
    } else {
      final widgets = <Widget>[];
      for (final sec in widget.store.sections) {
        final list = searched.where((s) => s.section == sec).toList();
        if (list.isEmpty) continue;
        widgets.add(_sectionHeader(sec, list.length));
        widgets.addAll(list.map((s) => _studentTile(c, s)));
      }
      final orphans = searched
          .where((s) => !widget.store.sections.contains(s.section))
          .toList();
      if (orphans.isNotEmpty) {
        widgets.add(_sectionHeader('غير مصنّف', orphans.length));
        widgets.addAll(orphans.map((s) => _studentTile(c, s)));
      }
      listArea = widgets.isEmpty
          ? const Empty(icon: Icons.person_search, text: 'لا توجد نتائج.')
          : ListView(
              padding: const EdgeInsets.symmetric(horizontal: 12),
              children: widgets,
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ListView(
              ...
              children: [
                for (final sec in chips)
                  Padding(
                    padding: const EdgeInsets.only(left: 8),
                    child: ChoiceChip(
                      label: Text(sec),
                      selected: filterSection == sec,
                      onSelected: (_) => setState(() => filterSection = sec),
                      selectedColor: AppColors.primary,
                      backgroundColor: Colors.white,
                      labelStyle: TextStyle(
                        color: filterSection == sec ? Colors.white : AppColors.primary,
                        fontWeight: FontWeight.bold,
                      ),
                    ),
                  ),
